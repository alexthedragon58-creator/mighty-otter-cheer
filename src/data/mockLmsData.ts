...
export const INITIAL_COURSES: Course[] = [
    ...
    flashcards: [
      {
        ...
        front: 'What does the "use client" directive declare?',
        back: 'It marks the module boundary between Server and Client component trees, instructing bundlers to prepare it for client hydration.'
      },
      {
        id: 'fc-2',
        category: 'Next.js 15 Actions',
        front: 'How do Server Actions prevent CSRF attacks in Next.js?',
        back: 'Next.js automatically generates and verifies an internal action ID and header check on incoming POST requests.'
      },
      {
        id: 'fc-3',
        category: 'AI SDK',
        front: 'What is the role of streamText() in Vercel AI SDK?',
        back: 'It delivers LLM token deltas progressively over HTTP SSE stream for instant typewriter UI responses.'
      }
    ],
    chapters: [
      {
        ...
        lessons: [
          {
            ...
            codeExercise: {
              id: 'ex-1-1',
              title: 'Exercise: Write a Server Action for Data Mutation',
              description: 'Define an asynchronous Server Action function named "createTodo" that validates a title parameter and returns an optimistic response object.',
              language: 'typescript',
              initialCode: `// Write your Server Action here\n'use server';\n\nexport async function createTodo(title: string) {\n  // TODO: Add validation and return { success: true, title }\n  \n}`,
              solutionCode: `'use server';\n\nexport async function createTodo(title: string) {\n  if (!title || title.trim().length === 0) {\n    throw new Error('Title is required');\n  }\n  return { success: true, id: Date.now().toString(), title: title.trim() };\n}`,
              hints: [
                'Ensure "use server" is at the top of the function or file.',
                'Check if the string is empty before returning the object.'
              ]
            }
          },
          {
    ...
    flashcards: [
      {
        ...
        front: 'What is Cosine Similarity measuring?',
        back: 'The cosine of the angle between two multi-dimensional vectors, evaluating orientation regardless of magnitude.'
      }
    ],
    chapters: [
      {
  ...
  },
  {
    ...
    requirements:<dyad-write path="src/data/mockLmsData.ts" description="Complete mock LMS data including courses, quizzes, code exercises, flashcards, webinars and discussions">
import { Course, LiveSession, ForumPost } from '../types/lms';
        ...
        lessons: [
          {
            ...
            codeExercise: {
              ...
              description: 'Define an asynchronous Server Action function named "createTodo" that validates a title parameter and returns an optimistic response object.',
              language: 'typescript',
              initialCode: `// Write your Server Action here\n'use server';\n\nexport async function createTodo(title: string) {\n  // TODO: Validate title not empty and return { success: true, title }\n  \n}`,
              solutionCode: `'use server';\n\nexport async function createTodo(title: string) {\n  if (!title || title.trim().length === 0) {\n    throw new Error('Title is required');\n  }\n  return { success: true, id: Date.now().toString(), title: title.trim() };\n}`,
              hints: [
                'Ensure "use server" is at the top of the function or file.',
                'Check if the string is empty before returning the object.'
  ...
  },
  {
    ...
    flashcards: [
      ...
      },
      {
        id: 'fc-202',
        category: 'Agents',
        front: 'What is LangGraph used for?',
        back: 'Building cyclical multi-agent workflows with state machines, conditional branches, and persistence.'
      }
    ],
    chapters: [
      {
