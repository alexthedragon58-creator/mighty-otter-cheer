...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const { currentUser, setRole, logout } = useLms();
  ...
  const getRoleBadge = (role: UserRole) => {
    switch (role) {
      case 'instructor':
        return <Badge className="bg-purple-100 text-purple-800 border-purple-200 text-[10px] font-bold">Instructor</Badge>;
      case 'admin':
        return <Badge className="bg-emerald-100 text-emerald-800 border-emerald-200 text-[10px] font-bold">Admin</Badge>;
      default:
        return <Badge className="bg-blue-100 text-blue-800 border-blue-200 text-[10px] font-bold">Student</Badge>;
    }
  };
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            {currentUser ? (
              <>
                ...
                <div className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 bg-amber-50 border border-amber-200 rounded-full text-amber-800 text-xs font-bold shadow-2xs">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="w-52 p-2">
                    <DropdownMenuLabel className="text-xs font-semibold text-slate-500">Switch Persona Mode</DropdownMenuLabel>
                    <DropdownMenuSeparator />
                    <DropdownMenuItem 
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <div className="flex items-center gap-2 pl-1 cursor-pointer group">
                      <img
                        ...
                        className="h-8 w-8 rounded-full ring-2 ring-indigo-500/20 object-cover group-hover:ring-indigo-500 transition-all"
                      />
                  ...
                  <DropdownMenuContent align="end" className="w-56 p-2">
                    ...
                    <DropdownMenuSeparator />
                    <DropdownMenuItem onClick={() => onTabChange('dashboard')} className="cursor-pointer text-xs flex items-center gap-2">
                      <BookOpen className="h-3.5 w-3.5 text-indigo-600" />
                      <span>My Learning Desk</span>
                    </DropdownMenuItem>
                    {currentUser.role === 'instructor' && (
                      <DropdownMenuItem onClick={() => onTabChange('studio')} className="cursor-pointer text-xs flex items-center gap-2">
                        <Layers className="h-3.5 w-3.5 text-purple-600" />
                        <span>Instructor Studio</span>
                      </DropdownMenuItem>
                    )}
                    {currentUser.role === 'admin' && (
                      <DropdownMenuItem onClick={() => onTabChange('admin')} className="cursor-pointer text-xs flex items-center gap-2">
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  className="rounded-xl text-xs font-semibold border-slate-200 hover:bg-slate-50 gap-1.5"
                >
